...
EVERFOREST_THEME="$DIR/theme/everforest.bash"
...
# Everforest Theme (ported from bspwm)
source_everforest() {
	cp "$EVERFOREST_THEME" "$CURRENT_THEME"
	source "$CURRENT_THEME"
	apply_palette_math
	accent="$color2"
	notify "Applying Everforest Theme…"
}
...
case "${1:-}" in
	...
	--pywal)
		...
		;;
	*)
		echo "Options: --default  --light  --everforest  --pywal"
		exit 1
		;;
esac

# Record which named theme is active so the waybar theme-cycle button
# (~/.config/sway/scripts/theme-cycle) and lightmode toggle know what to
# offer next, instead of guessing.
echo "${1#--}" > "$DIR/theme/.theme_name"
...
# No explicit wallpaper repaint needed here: apply_sway's `swaymsg reload`
# above re-applies the static fallback wallpaper apply_wallpaper just
# wrote to sway-output, but sway also re-runs every `exec_always` on
# reload -- including scripts/startup, which kills and relaunches the
# wallpaper-shuffle.sh loop, and that loop repaints immediately on its
# first iteration (see the script -- no initial sleep before the first
# paint_random). That already wins the race against the static fallback,
